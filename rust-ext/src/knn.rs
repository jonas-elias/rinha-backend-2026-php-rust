...
pub fn warmup() {
    ...
    unsafe {
        let cn = 14 * ds.k;
        for i in 0..cn {
            sink ^= (*ds.centroids.add(i)).to_bits() as u64;
        }
        for i in 0..=ds.k {
            sink ^= *ds.offsets.add(i) as u64;
        }
    }
...
unsafe fn scan_and_count(probes: &[usize], ds: &Dataset, q_vecs: &[__m256; 14]) -> u8 {
    ...
    let blocks_ptr = ds.blocks;
    let labels_ptr = ds.labels;

    for &ci in probes {
        let start = *ds.offsets.add(ci) as usize;
        let end = *ds.offsets.add(ci + 1) as usize;
        scan_blocks(
            q_vecs,
            blocks_ptr,
            labels_ptr,
