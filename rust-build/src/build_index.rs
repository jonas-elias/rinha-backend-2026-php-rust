...
fn main() {
    ...
    eprintln!("loading dataset from {:?}...", input);
    let (vectors, labels) = load_dataset(&input);
    let n = vectors.len();
    ...
    eprintln!("writing index to {:?}...", output);
    let t2 = Instant::now();
    write_index(&output, &vectors, &labels, &assignments, &centroids, n);
    eprintln!("  written in {:?}", t2.elapsed());
    eprintln!("total: {:?}", t0.elapsed());
}

pub fn load_dataset(path: &std::path::Path) -> (Vec<[f32; D]>, Vec<u8>) {
    let file = File::open(path).expect("open references file");
...
pub fn kmeans_plus_plus_init(vectors: &[[f32; D]], k: usize, seed: u64) -> Vec<[f32; D]> {
    let n = vectors.len();
...
pub fn update_centroids(vectors: &[[f32; D]], assignments: &[u16], centroids: &mut [[f32; D]]) {
    let k = centroids.len();
...
pub fn write_index(
    ...
    let file = File::create(output).expect("create index.bin");
    ...
    eprintln!(
        "  index.bin: {:.1}MB (padded_n={}, total_blocks={})",
